...
class Items extends Component {
  ...
  render() {
    ...
    const renderedItems = React.Children.map(items, (item) => {
      let active;
      if (item.props.value === this.props.selectValue) {
        active = <Option styleText={this.props.activeStyle}>√</Option>;
      }
      return (
    ...
    return (
      <View style={[styles.container, { top: positionY, left: positionX }]}>
        <ScrollView
          style={{ width: width - 2, height: height * 3, backgroundColor: '#ffffff' }}
