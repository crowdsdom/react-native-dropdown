...
class Select extends Component {
  ...
  _onPress() {
    ...
    if (optionListRef().state.show) {
      optionListRef()._onOverlayPress();
      return;
    }
  ...
  render() {
    ...
    let listRef = this.props.optionListRef();
    let indicator;
    if (listRef && listRef.state.show) {
      indicator = <Option styleText={[styleText, {fontFamily: 'Menlo', color: '#666666'}]}>▲</Option>;
    } else {
      indicator = <Option styleText={[styleText, {fontFamily: 'Menlo', color: '#666666'}]}>▼</Option>;
    }
